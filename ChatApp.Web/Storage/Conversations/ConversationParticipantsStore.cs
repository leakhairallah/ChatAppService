...
public class ConversationParticipantsStore : IConversationParticipantsStore
{
    ...
    {
        _logger.LogInformation("Getting the participants of conversations with user {userId}...", id);
    ...
    {
        _logger.LogInformation("Checking if conversation with id {id} exists...", id);
        ...
        if (response1 != null && response1.Count != 0)
        {
            if (response1.Count == 1)
            {
                ...
                if (participant == participant1)
                {
                    try
                    { 
                        _logger.LogInformation("Creating conversation between {participant1} and {participant2}...", participant1, participant2);
    ...
    {
        _logger.LogInformation("Getting conversations of participant {participant}...", participant);
